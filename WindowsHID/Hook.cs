...
public record MouseAllData
{
    public MSLLHOOKSTRUCT hookStruct { set; get; }
    public MouseMessagesHook mouseMessage { set; get; }
}

public static class SystemLevel_IO
{
    [DllImport("user32.dll", CharSet = CharSet.Auto, SetLastError = true)]
    public static extern IntPtr SetWindowsHookEx(HookType idHook, LowLevelMouseProc lpfn, IntPtr hMod, uint dwThreadId = 0);
...
public class MouseHook
{
    ...
    public static void addMouseCallback(EventHandler<MouseAllData> handler)
    {
    ...
    public event EventHandler<MouseAllData>? MouseAction;
    ...
    public void Start()
    {
        if (hookID == IntPtr.Zero)
        {
            ...
            {
                hookID = SystemLevel_IO.SetWindowsHookEx(HookType.WH_MOUSE_LL, LowLevelMouseProcCallback, SystemLevel_IO.GetModuleHandle(curModule.ModuleName));
            }
        }
    }
    ...
    private IntPtr LowLevelMouseProcCallback(int nCode, IntPtr wParam, IntPtr lParam)
    {
        void triggerEvent()
        {
            ...
            MouseAction?.Invoke(this, new MouseAllData() { mouseMessage = (MouseMessagesHook)wParam, hookStruct = hookStruct });
        }
        if (nCode >= 0)
        {
